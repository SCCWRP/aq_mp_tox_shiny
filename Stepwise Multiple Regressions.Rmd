...
aoc_setup <- aoc %>% 
  mutate(effect_f = factor(case_when(effect == "Y" ~ "Yes",
    effect == "N" ~ "No"),
    levels = c("No", "Yes"))) %>%
  replace_na(list(size.category = 0, shape = "Not Reported", polymer = "Not Reported", life.stage = "Not Reported", charge = "Not Reported")) %>% 
...
The response variable is the interaction between effect and effect score
...
## Remove Charge + Dose (volume)

Remove dose(volume) - highest p value 
...
## Remove Charge + Dose (volume) + Dose (mass)
...
#Model - Log Transformed + Interaction

Doses (count, mass and volume), size (continuous), and exposure duration are all log transformed given histogram results. The response variable is the interaction between effect and effect score.  
...
##Remove Particle Source + Organism Group
...
Since a linear model does not seem to work well for this data set. I'm going to explore other distribution that may fit the data better. 
