#Title: Concentration Range by Size Fraction for SSD
...
aoc_setup <- aoc_v1 %>% # start with original dataset
  mutate(size_f = factor(case_when(
    ...
    size.category == 4 ~ "100µm < 5mm",
    size.category == 5 ~ "100µm < 5mm",
    size.category == 0 ~ "Not Reported"),
    levels = c("1nm < 100nm", "100nm < 1µm", "1µm < 10µm", "10µm < 100µm", "100µm < 5mm", "Not Reported"))) %>% # creates new column with nicer names and order by size levels.
...
Particles <- aoc_z %>%
  #drop doses with no effect metric
  filter(!effect.metric == "NA") %>% 
  #Get rid of data that isn't chronic or acute
  filter(acute.chronic_f == "Unavailable") %>%    
  #filter for size fractions of interest
  filter(size_f %in% c("1µm < 10µm", "10µm < 100µm", "100µm < 5mm")) %>%   
  ggplot(aes(x = size_f, y = dose.particles.mL.master)) +
  geom_boxplot(fill = "darkmagenta") +
  scale_y_log10(limits = c(1e-5,1e12)) +
  theme(text = element_text(size=18)) +
  labs(title = "Raw Data",
       y = "Dose (Particles/mL)", x = "Size Category") 

Particles_Converted <- aoc_z %>%
  #drop doses with no effect metric
  filter(!effect.metric == "NA") %>% 
  #Get rid of data that isn't chronic or acute
  filter(!acute.chronic_f == "Unavailable") %>% 
  #Create new column where AF are applied
  mutate(new_dose = (dose.particles.mL.master/af.time)) %>% 
  #filter for size fractions of interest
  filter(size_f %in% c("1µm < 10µm", "10µm < 100µm", "100µm < 5mm")) %>%   
  ggplot(aes(x = size_f, y = new_dose)) +
  geom_boxplot(fill = "darkcyan") +
  scale_y_log10(limits = c(1e-5,1e12)) +
  theme(text = element_text(size=18)) +
  labs(title = "AF Applied",
       y = "Dose (Particles/mL)", x = "Size Category")

ggarrange(Particles, Particles_Converted)
...
Mass <- aoc_z %>%
  ...
  #filter for size fractions of interest
  filter(size_f %in% c("1µm < 10µm", "10µm < 100µm", "100µm < 5mm")) %>%   
  ggplot(aes(x = size_f, y = (dose.mg.L.master*1000))) +
  geom_boxplot(fill = "darkmagenta") +
  scale_y_log10(limits = c(1e-5,1e12)) +
  theme(text = element_text(size=18)) +
  labs(title = "Raw Data", y = "Dose (µg/L)", x = "Size Category", fill = "Exposure Duration")

Mass_converted <- aoc_z %>%
  #drop doses with no effect metric
  filter(!effect.metric == "NA") %>% 
  #Get rid of data that isn't chronic or acute
  filter(!acute.chronic_f == "Unavailable") %>%
  #Create new column where AF are applied
  mutate(new_dose = (dose.mg.L.master/af.time)) %>%
  #filter for size fractions of interest
  filter(size_f %in% c("1µm < 10µm", "10µm < 100µm", "100µm < 5mm")) %>%   
  ggplot(aes(x = size_f, y = (new_dose*1000))) +
  geom_boxplot(fill = "darkcyan") +
  scale_y_log10(limits = c(1e-5,1e12)) +
  theme(text = element_text(size=18)) +
  labs(title = "AF Applied", y = "Dose (µg/L)", x = "Size Category", fill = "Exposure Duration")

ggarrange(Mass, Mass_converted)
