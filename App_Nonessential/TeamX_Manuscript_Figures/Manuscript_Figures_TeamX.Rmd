...
b <- a %>%
  ...
  labs(y = "Number of Test Concentrations Reported", x = "Year", fill = "Concentration Metric") +
...
#Studies and measurements labels
a <- aoc_setup %>% 
      drop_na(dose.particles.mL.master) %>% #Drop data not reported in counts
      drop_na(effect_f) %>% #drop ECX values
      ...
      summarize(dose.particles.mL.master= quantile(dose.particles.mL.master, .1), # need for recognized "x"
                measurements = n(),
                studies = n_distinct(doi))
...
c <- b %>% 
  ...
  geom_text(data = a, 
                ...
                size = 3)+
  scale_x_log10(limits = c(-1,10000000000000000)) +
...
ggsave(filename = "Figure3.jpg",
       path = "App_Nonessential/TeamX_Manuscript_Figures/",
       plot = c, width = 8, height = 4, units = "in")

#Table

d <- b %>%
  select(doi, authors, year, species_f, life_f, shape_f, lvl1_f, lvl2_f, lvl3_f, dose.particles.mL.master, effect_f) %>% 
  rename("DOI" = doi, "Authors" = authors, "Publication Year" = year, 
         "Species" = species_f, "Life Stage" = life_f, "Shape" = shape_f, 
         "Broad Endpoint Category" = lvl1_f, "Specific Endpoint Category" = lvl2_f,
         "Endpoint" = lvl3_f, "Concentration (Particles/mL)" = dose.particles.mL.master, 
         "Effect Detected?" = effect_f) 

write.csv(d, "App_Nonessential/TeamX_Manuscript_Figures/Figure3_Data_SI.csv", row.names = FALSE)
...
b_labs <- aoc_setup %>% 
      ...
      filter(effect_f != "NA") %>% #drop ECX values
      group_by(size_f, effect_f) %>% # need to include so there's a recognized "y"
      summarize(dose.mg.L.master= quantile(dose.mg.L.master, .1), # need for recognized "x"
                measurements = n(),
                studies = n_distinct(doi))
...
d <- aoc_setup %>%
  ...
  filter(effect_f != "NA") %>% #drop ECX values
  drop_na(dose.mg.L.master|dose.particles.mL.master) %>%
  select(doi, authors, year, species_f, life_f, size_f, lvl1_f, lvl2_f, lvl3_f, 
         dose.particles.mL.master, dose.mg.L.master, effect_f) %>% 
  rename("DOI" = doi, "Authors" = authors, "Publication Year" = year, 
         "Species" = species_f, "Life Stage" = life_f, "Size Category" = size_f, 
         "Broad Endpoint Category" = lvl1_f, "Specific Endpoint Category" = lvl2_f,
         "Endpoint" = lvl3_f, "Concentration (Particles/mL)" = dose.particles.mL.master, 
         "Concentration (µg/mL)" = dose.mg.L.master,
         "Effect Detected?" = effect_f) 

write.csv(d, "App_Nonessential/TeamX_Manuscript_Figures/Figure4_Data_SI.csv", row.names = FALSE)
...
ssd_table <- aoc_intermediate %>% 
  mutate(dose_filter = ifelse(is.na(dose.particles.mL.master)&is.na(dose.mg.L.master), "N", "Y")) %>% 
  filter(dose_filter == "Y") %>% 
  select(doi, authors, year, species_f, life_f, size_f, lvl1_f, lvl2_f, lvl3_f, 
         dose.particles.mL.master, dose.mg.L.master, effect.metric) %>% 
  rename("DOI" = doi, "Authors" = authors, "Publication Year" = year, 
         "Species" = species_f, "Life Stage" = life_f, "Size Category" = size_f, 
         "Broad Endpoint Category" = lvl1_f, "Specific Endpoint Category" = lvl2_f,
         "Endpoint" = lvl3_f, "Concentration (Particles/mL)" = dose.particles.mL.master, 
         "Concentration (µg/mL)" = dose.mg.L.master, "Effect Metric" = effect.metric) 

write.csv(ssd_table, "App_Nonessential/TeamX_Manuscript_Figures/Figure5_Data_SI.csv", row.names = FALSE)
...
Group_size <- aoc_setup %>% 
  filter(org_f != "Mixed") %>% 
  filter(size_f != "Not Reported") %>% 
  group_by(org_f, size_f) %>% 
  mutate(count = n()) %>% 
  ungroup() %>% 
  group_by(org_f, size_f, count) %>% 
  summarise() %>% 
  ggplot(aes(x = reorder(org_f, count), y = count, fill = size_f)) +
  geom_col(position = position_stack(reverse = TRUE), color = "black") +
  scale_fill_manual(values = c('#d73027','#fc8d59','#fee090','#e0f3f8','#4575b4')) +
...
Group_size_table <- aoc_setup %>%
  ...
  group_by(org_f, size_f, count) %>% 
  summarise() %>%
  rename("Organism Group" = org_f, "Size Cateogry" = size_f, "Number of Observations" = count) 

write.csv(Group_size_table, "App_Nonessential/TeamX_Manuscript_Figures/Figure7A_Data_SI.csv", row.names = FALSE)


#Endpoint size plot
Endpoint_size <- aoc_setup %>%
  mutate(lvl1_f = ifelse(lvl1_f == "Behavioral, Sensory, Neurological", "Behavioral, Neurological", paste(lvl1_f))) %>% 
  filter(size_f != "Not Reported") %>% 
  group_by(lvl1_f, size_f) %>% 
  mutate(count = n()) %>% 
  ungroup() %>% 
  group_by(lvl1_f, size_f, count) %>% 
  summarise() %>% 
  ggplot(aes(x = reorder(lvl1_f, count), y = count, fill = size_f)) +
  geom_col(position = position_stack(reverse = TRUE), color = "black") +
  scale_fill_manual(values = c('#d73027','#fc8d59','#fee090','#e0f3f8','#4575b4')) +
  ylab("Number of Observations")+
...
Endpoint_size_table <- aoc_setup %>%
  ...
  group_by(lvl1_f, size_f, count) %>% 
  summarise() %>%
  rename("Endpoint Category" = lvl1_f, "Size Cateogry" = size_f, "Number of Observations" = count) 

write.csv(Endpoint_size_table, "App_Nonessential/TeamX_Manuscript_Figures/Figure7B_Data_SI.csv", row.names = FALSE)
