#Script for one-time data clean up to eliminate unnecessary parts of setup. 
...
aoc_setup <- aoc_v1 %>% 
  ...
  mutate(lvl1 = case_when(lvl1 == "alimentary.excretory" ~ "Alimentary, Excretory",
                                   ...
                                   lvl1 == "stress" ~ "Stress")) %>% 
  mutate(lvl2 = case_when(lvl2 == "abundance"~"Abundance",
  ...
  mutate(lvl3 = case_when(lvl3 == "11bhsd.mrnaexpression"~"11β HSD mRNA expression",
  ...
  mutate(acute.chronic = case_when(acute.chronic == "acute" ~ "Acute",
                                            acute.chronic == "chronic" ~ "Chronic")) %>% #factorize assesment factor time into chronic/acute
  mutate(tech.tier.zero = case_when(tech.tier.zero == "Fail" ~ "Red Criteria Failed",
                                             tech.tier.zero == "Pass" ~ "Red Criteria Passed")) %>% 
  mutate(risk.tier.zero = case_when(risk.tier.zero == "Fail" ~ "Red Criteria Failed",
  ...
  mutate(experiment.type = case_when(leachate.only == "Y" ~ "Leachate",
