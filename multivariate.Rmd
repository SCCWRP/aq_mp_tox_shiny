...
multiVar <- aoc_z %>% dplyr::select(#doi, size.category, 
  ...
  mutate_if(is.character, as.factor) %>% mutate(effect_10 = case_when(
     effect_f == "Y" ~ 1,
     effect_f == "N" ~ 0)) %>% 
                                 #  max.size.ingest.mm) %>%  #max ingestible size
  filter(!size_f == "Not Reported")  #take out not reported 
...
```{r fit results}
fitted <- predict(acuterf, acute_test[,-6], OOB = TRUE, type ="response")
misClasificError <- mean(fitted != acute_test$effect_f)
print(paste('Training Accuracy', 1 - misClasificError))
```
...
```{r fit results}
fitted <- predict(chronicrf, chronic_test[,-6], OOB = TRUE, type ="response")
misClasificError <- mean(fitted != chronic_test$effect_f)
print(paste('Training Accuracy', 1 - misClasificError))
```
...
misClasificError <- mean(fitted != all_test$effect_f)
print(paste('Training Accuracy', 1 - misClasificError))
```
...
```{r fit results}
fitted <- predict(nofilterrf, nofilter_test[,-6], OOB = TRUE, type ="response")
misClasificError <- mean(fitted != nofilter_test$effect_f)
print(paste('Training Accuracy', 1 - misClasificError))
```
...
```{r}
###CHRONIC
chronicpredictions <- as.data.frame(predict(chronicrf, chronic_test[,-6], type = "prob"))
...
####ACUTE
predictions <- as.data.frame(predict(acuterf, acute_test[,-6], type = "prob"))
# predict class and then attach test class
...
#### all
allpredictions <- as.data.frame(predict(allrf, all_test[,-6], type = "prob"))
# predict class and then attach test class
...
###nofilter
nofilterpredictions <- as.data.frame(predict(nofilterrf, nofilter_test[,-6], type = "prob"))
# predict class and then attach test class
...
roc.acute <- roc(ifelse(predictions$observed=="Yes", "Yes", "No"), as.numeric(predictions$Yes))

#chronic
roc.chronic <- roc(ifelse(chronicpredictions$observed=="Yes", "Yes", "No"), as.numeric(chronicpredictions$Yes))
#all
roc.all <- roc(ifelse(allpredictions$observed=="Yes", "Yes", "No"), as.numeric(allpredictions$Yes))

#nofilter
roc.nofilter <- roc(ifelse(nofilterpredictions$observed=="Yes", "Yes", "No"), as.numeric(nofilterpredictions$Yes))

allROC <- ggroc(roc.all) + labs(title = "Chronic and Acute",
                             subtitle = "Accuracy = 81.7%, n = 1510") + dark_theme_bw()

acuteROC <- ggroc(roc.acute, col = "red") + labs(title = "Acute",
                                              subtitle = "Accuracy = 84.8%, n = 883") + dark_theme_bw()

chronicROC <- ggroc(roc.chronic, col = "blue") + labs(title = "Chronic",
                                                   subtitle = "Accuracy = 67.3%, n = 452") + dark_theme_bw()
 
nofilterROC <- ggroc(roc.nofilter, col = "green") + labs(title = "Entire Dataset",
                                                   subtitle = "Accuracy = 79.9%, n = 4615") + dark_theme_bw()
require(gridExtra)
grid.arrange(nofilterROC, allROC, acuteROC, chronicROC,
             ncol = 4)
```
```{r}
require(pROC)
...
require(ggdark)
require(ggsci)
ggroc(list(all = roc.nofilter, organisms = roc.all, acute = roc.acute, chronic = roc.chronic), aes = c("linetype", "color")) +
  labs(title = "ROC Curves for Aquatic Toxicity RF",
       subtitle = "n = 4615",
       color = "Dataset",
       linetype = "Dataset") +
  scale_color_tron()+
...
acute_density <- acute %>% 
  ...
  state_density_2d(aes(fill = stat(effect.score)),
                       geom = "polygon",
