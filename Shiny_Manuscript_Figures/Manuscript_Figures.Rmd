---
title: 'Team X Manuscript Figures: Aquatic Organisms'
...
aoc_setup <- aoc_v1 %>% # start with original dataset
  mutate(size_f = factor(case_when(
    ...
    levels = c("1nm < 100nm", "100nm < 1µm", "1µm < 10µm", "10µm < 100µm", "100µm < 1mm", "1mm < 5mm", "Size Not Reported"))) %>% # creates new column with nicer names and order by size levels.
...
```{r collapsible tree example, include=FALSE}

collapsibleTree(aoc_setup, root = "Aquatic Organisms Database", hierarchy = c("lvl1_f", "lvl2_f", "lvl3_f", "bio_f"),
                    fontSize = 16, zoomable = FALSE,    
    fill = c(
      # The root
      "seashell",
      # lvl1
      rep("turquoise", length(unique(aoc_setup$lvl1_f))),
      # lvl2
      rep("palegreen", length(unique(paste(aoc_setup$lvl1_f, aoc_setup$lvl2_f)))),
      # lvl3
      rep("hotpink", length(unique(paste(aoc_setup$lvl1_f, aoc_setup$lvl2_f, aoc_setup$lvl3_f)))),
      # bio org
      rep("orchid", length(unique(paste(aoc_setup$lvl1_f, aoc_setup$lvl2_f, aoc_setup$lvl3_f, aoc_setup$bio_f))))))
    

```

```{r endpoints and organism groups, fig.width = 15, include=FALSE}
library(viridis)
a <- aoc_setup %>% 
  filter(effect_f != "NA") %>%
  count(lvl1_f, org_f) %>% #how many obs of each organism group within each lvl1
  group_by(lvl1_f) %>%
  mutate(n_lvl1 = sum(n)) %>% #how many obs within each lvl1
  ungroup() %>%   
  ggplot(aes(x = fct_reorder(lvl1_f, n_lvl1), y = n, group = n, fill = org_f)) +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single"), color = "black", width = 1) +
  scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 10)) +
  scale_y_continuous(expand = c(0,0)) +
  scale_fill_manual(values= wes_palette("Zissou1", type = "continuous", 13)) +
  theme_classic() +
  theme(text = element_text(size = 16))+
  labs(y = "Number of Endpoints in Database", x = "Endpoint Category", fill = "Organism\nGroup")

plot(a)

```

```{r particle characteristics, include=FALSE}
...
particle <- aoc_setup %>%
  ...
  mutate(poly_pct = (sum(poly_f != "Not Reported")/size_count)*100) %>% 
  #percentage that report shape
  mutate(shape_pct = (sum(shape_f != "Not Reported")/size_count)*100) %>%
  ...
  ungroup() %>% 
  group_by(size_f, poly_pct, shape_pct, density_pct, charge_pct, fxngroup_pct, sizevalid_pct, shapevalid_pct, polyvalid_pct) %>% 
  summarise() %>% 
...
All_data <- data.frame(particle, row.names = 1)%>% 
  arrange("1nm < 100nm", "100nm < 1µm", "1µm < 10µm", "10µm < 100µm", "100µm < 1mm", "1mm < 5mm", "Size Not Reported")
