...
library(collapsibleTree) #For creating collapsable tree figure
...
#Assign colors to unqiue biological levels of organization
aoc_clean$Color <- aoc_clean$bio_f
levels(aoc_clean$Color) <- colorspace::rainbow_hcl(5)
  
#### Set Up #### 

collapsibleTree(aoc_clean, root = "Aquatic Organisms Database", hierarchy = c("lvl1_f", "lvl2_f", "lvl3_f", "bio_f"),
                fontSize = 12, fillByLevel = TRUE,
                fill = c(
                  # The root
                  "seashell",
                  # lvl1
                  rep("turquoise", length(unique(aoc_clean$lvl1_f))),
                  # lvl2
                  rep("palegreen", length(unique(paste(aoc_clean$lvl1_f, aoc_clean$lvl2_f)))),
                  # lvl3 
                  rep("hotpink", length(unique(paste(aoc_clean$lvl1_f, aoc_clean$lvl2_f, aoc_clean$lvl3_f)))),
                  # bio org
                  rep("orchid", length(unique(paste(aoc_clean$lvl1_f, aoc_clean$lvl2_f, aoc_clean$lvl3_f, aoc_clean$bio_f))))))
