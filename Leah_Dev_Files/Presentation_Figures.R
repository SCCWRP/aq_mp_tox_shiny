...
#This is the same tidying and setup as found in app.R as of 3/17/21
# Master dataset for scatterplots - for Heili's tab.
aoc_work <- aoc %>% # start with original dataset
...
Study <- aoc_work %>%
  group_by(org_f, shape_f, article) %>% 
  summarise() %>% 
  mutate(study_number = n_distinct(org_f, article)) %>%
  ungroup() %>% 
  group_by(org_f, shape_f, study_number) %>% 
  summarise()  
  ggplot(aes(x = org_f, y = study_number, fill = shape_f)) +
  geom_col()+ 
  theme_classic() +
  scale_fill_manual(values = c("mediumpurple1", "gray87", "gray87", "gray87")) +
  scale_color_manual(values = c("mediumpurple1", "gray87", "gray87", "gray87")) +
  theme(text = element_text(size = 18),
        legend.position="none", 
        axis.text = element_text(color = "black", angle = 45, vjust = 0.5, hjust=1)) +
  labs(x = "Organism Group", y = "Number of Studies")
...
Fiber <- aoc_work %>%
  ...
  filter(effect_f != "NA") %>% 
  group_by(org_f, dose.particles.mL.master, article) %>% 
  summarise()   
  ...
  geom_point(alpha = 0.8, size = 5, fill = "darkcyan", color = "darkcyan") +
  theme_classic() +
  scale_x_log10() +
  theme(text = element_text(size = 18),
  ...
  labs(caption = "Microplastic Fibers: Tested Doses", x = "Concentration (particles/mL)", y = "Organism Group")
