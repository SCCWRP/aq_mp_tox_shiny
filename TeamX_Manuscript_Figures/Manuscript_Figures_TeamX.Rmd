...
```{r Ludo, warning = FALSE, message = FALSE, fig.height = 7, fig.width = 12}
...
a <- aoc_setup %>% 
  ...
  ggplot(aes(y = shape_f, x = dose.mg.L.master, fill = effect_f)) +
  geom_boxplot(alpha = 0.8) +
  scale_x_log10() +
  labs(y = "Morphology", x = "mg/L", fill = "Effect") +
  theme_classic() +
...
c <- aoc_setup %>% 
  filter(environment != "Terrestrial") %>% #drop terrestrial organisms
  filter(!poly_f %in% c("Not Reported", "Polyethylene Vinyl Acetate", "Polycarbonate", "Latex")) %>% 
  filter(effect_f != "NA") %>% #drop ECX values
  filter(org_f %in% c("Crustacea", "Fish", "Mollusca")) %>%  #Select relevant organism groups
  filter(lvl2_f %in% c("Growth", "Body Condition", "Development", "Mortality", "Reproduction")) %>% 
  ggplot(aes(y = poly_f, x = dose.mg.L.master, fill = effect_f)) +
  geom_boxplot(alpha = 0.8) +
  scale_x_log10() +
  labs(y = "Polymer", x = "mg/L", fill = "Effect") +
  theme_classic() +
  theme(text = element_text(size = 16)) +
...
d <- aoc_setup %>% 
  ...
  filter(org_f %in% c("Crustacea", "Fish", "Mollusca")) %>%  #Select relevant organism groups
  filter(lvl2_f %in% c("Growth", "Body Condition", "Development", "Mortality", "Reproduction")) %>% 
  ggplot(aes(y = poly_f, x = dose.particles.mL.master, fill = effect_f)) +
  geom_boxplot(alpha = 0.8) +
  scale_x_log10() +
  labs(y = "Polymer", x = "particles/mL", fill = "Effect") +
  theme_classic() +
  theme(text = element_text(size = 16)) +
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 10))
...
b <- a %>%
  ...
  labs(y = "Frequency", x = "Year", fill = "Dose Metric") +
  theme(text = element_text(size = 16))

plot(b)

#Find studies that use multiple sizes
c <- aoc_setup %>% 
  group_by(doi, size.length.um.used.for.conversions) %>% 
  summarise() %>% 
  ungroup() %>% 
  count(doi)

#Find studies that use multiple shapes
d <- aoc_setup %>% 
  group_by(doi, shape_f) %>% 
  summarise() %>% 
  ungroup() %>% 
  count(doi)

#Find studies that use multiple polymers
e <- aoc_setup %>% 
  group_by(doi, poly_f) %>% 
  summarise() %>% 
  ungroup() %>% 
